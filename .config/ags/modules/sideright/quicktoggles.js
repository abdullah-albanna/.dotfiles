...
export const ModuleNightLight = async (props = {}) => {
  // Check if 'hyprshade' is available; if not, return null.
  ...
  return Widget.Button({
    ...
    onClicked: async (self) => {
      ...
      try {
        if (self.attribute.enabled) {
          await Utils.execAsync("hyprshade on blue-light-filter");
        } else {
          await Utils.execAsync("hyprshade off");
          // Disable the button until the command fully terminates to avoid race conditions.
          self.sensitive = false;

          const interval = setInterval(async () => {
            try {
              const output = await Utils.execAsync("hyprshade current");
              if (output.trim() === "") {
                self.sensitive = true;
    ...
    setup: (self) => {
      setupCursorHover(self);
      // Check current status of 'hyprshade' and set button state accordingly.
      self.attribute.enabled = !!exec("hyprshade current");
      self.toggleClassName("sidebar-button-active", self.attribute.enabled);
    },
